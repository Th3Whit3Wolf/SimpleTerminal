...
xhints(void)
{
	XClassHint class = {opt_name ? opt_name : "ubunstu",
	                    opt_class ? opt_class : "Ubunstu"};
...
resource_load(XrmDatabase db, char *name, enum resource_type rtype, void *dst)
{
	...
	snprintf(fullname, sizeof(fullname), "%s.%s",
			opt_name ? opt_name : "ubunstu", name);
	snprintf(fullclass, sizeof(fullclass), "%s.%s",
			opt_class ? opt_class : "Ubunstu", name);
...
main(int argc, char *argv[])
{
	...
	case 'w':
		...
		break;
	case 'v':
		die("%s " VERSION " (c) 2010-2016 Ubunstu engineers\n", argv0);
		break;
	default:
		usage();
	} ARGEND;
...
run:
	...
	if (!opt_title)
		opt_title = (opt_line || !opt_cmd) ? "Ubunstu" : opt_cmd[0];
